...
Const $SHORT = 8 ; 4 bytes ( 8 stellen )
Const $LONG = 16 ; 8 bytes ( 16 stellen )
...
Func msg_ToStr($id=0)
	...
	for $i = 1 to $PACKET[$id][0]
		$str &= $PACKET[$id][$i]
	Next
	$str = StringReplace($str,"XXXXXXXX",_RotateHex(Hex((StringLen($str)/2)-1)))
	Return $str
...
Func msg_getByte($count=1,$id=0)
	...
	$PACK[$id][0] = StringRight($str,StringLen($str)-$count*2)
	Return $PACK[$id][$i]
EndFunc
